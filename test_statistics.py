...
class StatisticsTest(TestCase):
    ...
    def test_average_typical_values(self):
        ...
        self.assertEqual(average([1, 2, 3, 4, 5]), 3.0)  # Typical case
        self.assertEqual(average([10.0, 20.0, 30.0]), 20.0)  # All floats
        self.assertEqual(average([1, 1, 1]), 1.0)  # All same values

    def test_average_empty(self):
        """Test average with an empty list."""
        with self.assertRaises(ValueError):
            average([])  # Assuming default average for empty list is 0.0

    def test_average_single_value(self):
        """Test average with a single value."""
        self.assertEqual(average([5]), 5.0)  # Single value

    def test_average_large_numbers(self):
        """Test average with very large numbers."""
        self.assertEqual(average([1e6, 1e6 + 1, 1e6 - 1]), 1e6)  # Large numbers

    def test_average_negative_numbers(self):
        """Test average with negative numbers."""
        self.assertEqual(average([-1, -2, -3, -4]), -2.5)  # Negative values

    def test_average_mixed_numbers(self):
        """Test average with mixed int and float numbers."""
        self.assertEqual(average([1, 2.5, 3, 4.75]), 2.8125)  # Mixed int and float
    ...
    def test_variance_large_values(self):
        ...
        self.assertAlmostEqual(2.6666666666666668e+16, variance([1e8, 1e8 + 2e8, 1e8 - 2e8]))
    ...
    def test_variance_floats(self):
        ...
        self.assertAlmostEqual(variance([1.5, 2.0, 2.5]), 0.16666666666666666)

    def test_variance_large_dataset(self):
        """Test variance with a large dataset."""
        large_data = [i for i in range(1000)]
        expected_variance = sum((x - 499.5) ** 2 for x in large_data) / len(large_data)
        self.assertAlmostEqual(variance(large_data), expected_variance)

    def test_variance_integers_and_floats(self):
        """Test variance with a mix of integers and floats."""
        self.assertAlmostEqual(variance([1, 2.5, 3]), 0.7222222222222222)
    ...
    def test_stdev_negative_numbers(self):
        ...
        self.assertAlmostEqual(2.8284271247461903, stdev([-10, -2, -8, -4, -6]))

    def test_stdev_large_numbers(self):
        """Test standard deviation with very large numbers."""
        self.assertAlmostEqual(sqrt(2.6666666666666668e16), stdev([1e8, 1e8 + 2e8, 1e8 - 2e8]))
    ...
    def test_stdev_floats(self):
        ...
        self.assertAlmostEqual(sqrt(variance([1.5, 2.0, 2.5])), stdev([1.5, 2.0, 2.5]))
